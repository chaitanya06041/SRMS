...
class resultClass:
    def __init__(self,root):
        ...
        txt_course=Entry(self.root,textvariable=self.var_course,font=("goudy old style",20,"bold"),bg="lightyellow",state="readonly").place(x=280,y=160,width=320)
        txt_marks=Entry(self.root,textvariable=self.var_marks,font=("goudy old style",20,"bold"),bg="lightyellow",state="readonly").place(x=280,y=160,width=320)
        txt_full_marks=Entry(self.root,textvariable=self.var_full_marks,font=("goudy old style",20,"bold"),bg="lightyellow",state="readonly").place(x=280,y=160,width=320)
        ...
        btn_clear=Button(self.root,text="clear",font=("times new roman",15),bg="lightgray",activebackground="lightgrayl",cursor="hand2",command=self.clear).place(x=430,y=420,width=120,height=35)
        ...
        try:
           if self.var_name.get()=="":
             ...
             cur.execute("select*from result whereroll=? and course=?",(self.var_roll.get(),self.var_course.get()))
             row=cur.fetchone()
           if row!=None:
